...
**Validation status:** 2.5.0 release-gate testing is complete. The pre-handshake Jotunn/Epic Loot incomplete-client reproduction and the multi-root plugin/patcher/config/server-only validation passed before the v2.5.0 release. v2.5.0 was published unsigned. The September 2026 SignPath Foundation application was declined because the project did not yet have enough external public-adoption/trust signals; code signing remains an optional future enhancement rather than a release blocker.
...
Current 2.5 peers additionally advertise `bundle-pipeline1`. The client can request up to 128 chunks in one window; the server still keeps each individual `AMS4_BundleBatch` message at roughly 384 KiB or less, but queues several of those messages back-to-back before waiting for the next client request. This removes the one-request-per-384-KiB pacing bubble without increasing a single Steam message beyond the conservative batch bound.

Fallback order is: `bundle-pipeline1` -> `bundle-batch1` -> `bundle-window1` -> original single-chunk AMS4. Older AMS4 clients and servers therefore continue to interoperate without a protocol-version bump.

Live testing on a gigabit LAN isolated a second bottleneck below AutoModSync's framing: Valheim's Steam transport starts its send-rate bounds near 153600 B/s, closely matching the observed ~0.1-0.15 MiB/s transfer ceiling even when raw TCP/iperf reaches line rate. The first 2.5 attempt raised only `SendRateMax`, but a live test remained pinned near the old rate. The next test proved Steam was then holding `m_nSendRateBytesPerSecond` **exactly at the configured 1 MiB/s minimum for the entire bundle**, while the maximum had already been raised successfully. The current path therefore resolves the actual socket directly from the active ZRpc, unwraps known socket decorators, and uses development defaults of `SendRateMax=32 MiB/s`, `SendRateMin=8 MiB/s`, and `SendBuffer=16 MiB`. The exact earlier development defaults are migrated automatically; administrator-customized values are preserved. Every previous live value is captured and only settings AutoModSync actually changed are restored when the bundle completes or aborts. Non-Steam/PlayFab paths simply skip this optimization.
...
Code signing is tracked separately. v2.5.0 was published unsigned. The September 2026 SignPath Foundation application was declined for insufficient public-adoption/trust signals; the project may revisit trusted Authenticode signing after broader adoption or through another signing path.
