...
Live testing on a gigabit LAN isolated a second bottleneck below AutoModSync's framing: Valheim's Steam transport starts its send-rate bounds near 153600 B/s, closely matching the observed ~0.1-0.15 MiB/s transfer ceiling even when raw TCP/iperf reaches line rate. The first 2.5 attempt raised only `SendRateMax`, but a live test remained pinned near the old rate. The current path therefore resolves the **actual socket directly from the active ZRpc**, unwraps known socket decorators, and tunes that exact Steam connection for the bundle: default `SendRateMax=8 MiB/s`, `SendRateMin=1 MiB/s`, and `SendBuffer=8 MiB`. All three are configurable, every previous value is captured, and only settings AutoModSync actually changed are restored when the bundle completes or aborts. Non-Steam/PlayFab paths simply skip this optimization.
