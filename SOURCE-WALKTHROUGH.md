...
Server-browser presence is intentionally separate from synchronization discovery. On a dedicated Steam backend the server can advertise `automodsync` and `automodsync_protocol` as ordinary Steam server rules after SteamGameServer reports a logged-on state. Those rules carry only product capability/version information and do not contain the server signing fingerprint, private identity, endpoint, or player identifiers. Before the client modifies Valheim's browser rows, the development-only `phase7-test-server-browser-probe.once` path captures the exact live `ServerListGui` row hierarchy and bounded field metadata so the final logo badge can be attached to a stable existing row anchor instead of hard-coded screen coordinates. Production badge identity comes from `m_serverListElements`, not by zipping `ListRoot` children with `m_filteredList`: one wrapper must yield both the rendered ServerElement and its own ServerJoinData before a passive Steam-rule result can affect that row. Ambiguous/unbound rows render no badge.
