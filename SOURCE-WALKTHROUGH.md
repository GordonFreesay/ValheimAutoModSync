...
First-contact trust is shown as an in-game AutoModSync prompt rather than a blocking native Windows MessageBox. The signed manifest and exact active `ZRpc` are retained while the player verifies the fingerprint, allowing Valheim's networking loop to continue. While the prompt is pending, AutoModSync temporarily forces the Unity cursor visible and unlocked on every frame because Valheim's connection state otherwise hides/locks its sword pointer; Enter and Escape are trust/cancel fallbacks handled through the active IMGUI `Event.current` so the client does not add a dependency on Unity's legacy Input module. A normal decision restores the cursor mode Valheim owned before the prompt. Trust acceptance is valid only for that same still-connected, signature-verified AMS session. If the recognized socket dies while trust or package preparation is pending, AutoModSync discards the protected session, clears transient UI without replaying the held vanilla handshake, and leaves an unlocked visible pointer for the returned menu state.
