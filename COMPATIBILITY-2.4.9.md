# AutoModSync 2.4.9 compatibility and signing plan
...
2.4.9 moves discovery to the outgoing connection's `ZNet.OnNewConnection` path and temporarily holds only the vanilla `ServerHandshake` RPC.
...
Do not publish 2.4.9 until the Jotunn/Epic Loot reproduction that exposed the 2.4.8 ordering issue succeeds from a deliberately incomplete client.
