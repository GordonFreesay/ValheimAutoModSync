...
try {
    ...
    Assert-Contains $devBuild 'AutoModSync.IdentityDisplay.cs' 'Dev build identity helper'
    Assert-Contains $devBuild 'verify-no-pii.ps1' 'Dev build PII gate'
    ...
    Assert-Contains $releaseBuild '/resource:"%BRANDING_PNG%",ValheimAutoModSync.Branding.Logo.png' 'Release build'
    Assert-Contains $releaseBuild '/win32icon:"%APPLYICO%"' 'Release build'
    Assert-Contains $releaseBuild 'AutoModSync.IdentityDisplay.cs' 'Release build identity helper'
    Assert-Contains $releaseBuild 'verify-no-pii.ps1' 'Release build PII gate'
    Assert-Contains $releaseBuild 'ValheimAutoModSyncInstaller.ico' 'Standalone installer package'
