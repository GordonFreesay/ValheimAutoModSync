...
- Live telemetry showed Steam holding the bundle connection exactly at the configured 1 MiB/s minimum for the full transfer. The development defaults now use an 8 MiB/s temporary minimum, 32 MiB/s maximum, and 16 MiB reliable buffer, with automatic migration from the earlier exact 2.5 test defaults.
