...
| `Source/ValheimAutoModSync.Client.cs` | Replaces the blocking Win32 first-contact fingerprint MessageBox with an in-game non-blocking trust prompt so Valheim/ZRpc networking continues while the user verifies identity. Binds acceptance to the same live signature-verified RPC/manifest, and clears transient AMS UI/state fail-closed if that recognized connection dies before trust or synchronization completes. |
...
- Phase 3 implementation: **partial runtime validation** — a 60-file / 313.4 MiB near-bare request hit the retained cache with `prepare=0.000 s`, proving join-time cache reuse. Startup MISS/build timing, WAIT-HIT concurrency, invalidation, TTL/budget eviction, and the new non-blocking trust/lost-connection behavior still require runtime validation.
