...
| `Source/ValheimAutoModSync.Client.cs` | Replaces the blocking Win32 first-contact fingerprint MessageBox with an in-game non-blocking trust prompt so Valheim/ZRpc networking continues while the user verifies identity. Binds acceptance to the same live signature-verified RPC/manifest, clears transient AMS UI/state fail-closed if that recognized connection dies, and temporarily forces a visible/unlocked cursor (plus IMGUI Event-based Enter/Escape fallbacks, avoiding an InputLegacyModule dependency) so the prompt remains actionable while Valheim's connection screen normally hides/locks its sword pointer. |
...
- Phase 3 implementation: **partial runtime validation** — a 60-file / 313.4 MiB near-bare request hit the retained cache with `prepare=0.000 s`, proving join-time cache reuse. Recognized-session disconnect cleanup while trust UI is visible also passed. Startup MISS/build timing, WAIT-HIT concurrency, invalidation, TTL/budget eviction, delayed trust acceptance, and the new cursor-actionable trust prompt still require runtime validation.
