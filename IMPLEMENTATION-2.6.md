...
| `Source/ValheimAutoModSync.Client.cs` | Replaces the blocking Win32 first-contact fingerprint MessageBox with an in-game non-blocking trust prompt so Valheim/ZRpc networking continues while the user verifies identity. Binds acceptance to the same live signature-verified RPC/manifest, clears transient AMS UI/state fail-closed if that recognized connection dies, and temporarily forces a visible/unlocked cursor (plus Enter/Escape fallbacks) so the prompt remains actionable while Valheim's connection screen normally hides/locks its sword pointer. |
