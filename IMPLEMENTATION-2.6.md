...
| File | 2.6 reason |
| --- | --- |
| `Source/AutoModSync.OwnershipState.cs` | Adds strict fingerprint-scoped ownership ledgers for files AutoModSync actually installed. Matching pre-existing local files are not claimed. Ledger entries carry fixed kind/path/size/SHA-256 and reject protected config names. |
| `Source/ValheimAutoModSync.Client.cs` | Builds desired ownership only from a verified trusted manifest, claims unowned paths only when AMS must install/replace them, schedules exact-digest stale deletions for the same server, preserves modified stale/local files while relinquishing ownership, supports delete-only apply/restart, and publishes metadata-only relinquishment without live-file mutation. No AMS wire protocol change is introduced. |
...
| `build-dev.bat` | Compiles the ownership-state source into the development client and Apply helper. |
| `build-release.bat` | Compiles the same ownership-state source into the release-path client/helper without enabling development fault hooks. No release artifact has been authorized or built. |
...
- Phase 6 implementation: **complete in source; isolated ownership/apply validation passed, integrated client lifecycle pending** — ownership is fingerprint-scoped and acquired only by actual AMS writes, not by coincidentally matching local files. A later signed omission from that same trusted server can schedule deletion only when the live bytes still equal the exact last-owned digest; modified stale files are preserved and ownership is relinquished. Write/delete operations share the PREPARED/COMMITTED transaction, new journals use `AMSTXN2` while recovery still reads `AMSTXN1`, and ownership itself is published only after COMMITTED (including interrupted COMMITTED recovery). Windows CI passed the expanded 7/7 isolated ownership/helper cases, and a separate current-helper Phase 2 transaction regression rerun passed 4/4. Local integrated build plus live acquire/remove/rename/modified-local/zero-delta behavior remain to be observed before Phase 6 closes.
