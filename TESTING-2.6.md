...
- [ ] After resumed completion the full ZIP SHA-256, ZIP extraction checks, transactional apply, restart, and normal reconnect still pass exactly as for a fresh transfer.
...
- A one-client live Phase 4 run on 2026-09-23 completed through apply/restart/reconnect: the post-restart client loaded AutoModSync 2.6, queued and dispatched the one-shot reconnect, then reported that client mods matched the trusted server and released the held Valheim handshake. However, the supplied "before reconnect" capture ends during BepInEx chainloader startup before AutoModSync itself loads, and the post-restart LogOutput no longer contains the pre-restart transfer process. Therefore the final apply/reconnect half is observed, but the live nonzero resume offset, server exact-prefix acceptance, and dead-peer cleanup checkboxes remain pending until the server log or an equivalent preserved transfer log shows those lines.
