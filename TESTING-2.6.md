...
- [ ] `build-dev.bat` compiles the integrated client/server runtime with `AutoModSync.TransferScheduler.cs`.
- [ ] A real one-client delta sync is admitted through the scheduler, receives the bundle, verifies/applies/restarts/reconnects normally, and restores temporary Steam settings.
...
- Phase 5 deterministic scheduler CI on 2026-09-23 initially exposed a fairness bug: the round-robin cursor advanced when the aggregate token bucket was short, allowing refill boundaries to skip a peer's turn. The scheduler was corrected to retain the current peer until enough tokens exist for its fixed-size grant. The rerun on Windows Server 2025 then passed all 6/6 checks: 8-peer/4-slot admission, FIFO promotion, aggregate rate+burst enforcement, four-peer fairness, blocked-grant refund behavior, and queued-peer removal. This validates the production scheduler policy class independently of Valheim; integrated ZRpc/Steam runtime validation is still pending.
