...
- The same retest exposed an input-state issue: Valheim hides/locks its sword pointer during connection, so the new in-game trust buttons rendered but were not clickable. Source now temporarily owns the cursor while the trust prompt is pending (visible + unlocked, reasserted each frame), restores Valheim's previous cursor state after a normal decision, forces an unlocked visible pointer after recognized-session loss, and adds Enter/Escape trust/cancel fallbacks. The first implementation used `UnityEngine.Input`, which the existing Visual Studio harness does not reference and therefore failed compile with CS0103; that dependency was removed in favor of `Event.current` / IMGUI key events, avoiding any new Unity assembly reference. Pointer interaction and the corrected build are **not yet runtime-revalidated**.
